...
import { fetchPostById, fetchPosts } from '../../postsActions/PostsActions'
...
const SelectedPost: React.FC = () => {
  ...
  const post = useAppSelector((state: RootState) => state.posts.selectedPost)
  const posts = useAppSelector((state: RootState) => state.posts.allPosts) 
  ...
  const currentPost = posts?.find((p: PostType) => String(p.id) === id)
  ...
  const handlePreviousPost = () => {
    if (!posts) return;
    const currentIndex = posts.findIndex((p: PostType) => String(p.id) === id)
    if (currentIndex > 0) {
      const previousPostId = posts[currentIndex - 1].id
